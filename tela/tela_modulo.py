...
class Panel(QWidget):
    ...
    # Relay
    def Update_Panel(self, timer_state, q_image, eraser, sof_1, sof_2, sof_3, can_rotation, can_zoom, exposure, gamma, mirror, wrap, doc_width, doc_height, angle, size, doc_p1_x, doc_p1_y, doc_p2_x, doc_p2_y, doc_p3_x, doc_p3_y, doc_p4_x, doc_p4_y, width, height, ui_d_ref, delta_new, gui_hor_norm, gui_ver_norm, panel_width, panel_height, gray_natural, gray_contrast):
        self.timer_state = timer_state
        self.q_image = q_image.scaled(self.width, self.height, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        self.angle = angle
        self.size = size
        self.doc_p1_x = doc_p1_x
        self.doc_p1_y = doc_p1_y
        self.doc_p2_x = doc_p2_x
        self.doc_p2_y = doc_p2_y
        self.doc_p3_x = doc_p3_x
        self.doc_p3_y = doc_p3_y
        self.doc_p4_x = doc_p4_x
        self.doc_p4_y = doc_p4_y
        self.width = width
        self.height = height
    ...
    def drawColors(self, event):
        ...
        if self.wrap == True:
            if self.eraser == True:
                painter.setPen(QPen(QColor(self.gray_natural), 10, Qt.SolidLine, Qt.SquareCap, Qt.MiterJoin))
            else:
                painter.setPen(QPen(QColor(self.gray_contrast), 10, Qt.SolidLine, Qt.SquareCap, Qt.MiterJoin))
            square = QPainterPath()
        ...
        if self.timer_state == 1:
            ...
            # Draw QImage
            if self.mirror == True:
                painter.drawImage(0,0, self.q_image.mirrored(True, False))
            else:
...
class Record_old(QMainWindow):
    ...
    def Window_Show(self):
        ...
        self.setGeometry(self.top, self.left, self.size_x, self.size_y)
        self.label = QLabel(self)
        self.label.setGeometry(0,0, self.resolution_x,self.resolution_y)
        self.widget = QWidget(self)
        self.widget.setGeometry(0,0, self.size_x,self.size_y)
        self.show()
    def Close(self):
        self.close()

    def Update(self, resolution_x, resolution_y, q_image):
        self.resolution_x = resolution_x
        self.resolution_y = resolution_y
        self.pixmap = QPixmap.fromImage(q_image, Qt.AutoColor)
        self.label.setPixmap(self.pixmap)
        self.label.setGeometry(self.origin_x, self.origin_y, self.resolution_x, self.resolution_y)
        self.widget.setGeometry(0, 0, self.size_x, self.size_y)

    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QtGui.QPainter.Antialiasing, True)
        # Image Backdrop for Invisibility
        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(QColor(127,127,127)))
        painter.drawRect(self.origin_x, self.origin_y, self.resolution_x, self.resolution_y)

    def resizeEvent(self, event):
        self.size_x = self.size().width()
        self.size_y = self.size().height()
        self.origin_x = (self.size_x * 0.5) - (self.resolution_x * 0.5)
        self.origin_y = (self.size_y * 0.5) - (self.resolution_y * 0.5)
...
class Record(QMainWindow):
    ...
    def Window_Show(self):
        self.setWindowTitle(self.title)
        self.setGeometry(self.top, self.left, self.resolution_x, self.resolution_y)
        self.label = QLabel(self)
        self.label.setGeometry(0,0, self.resolution_x,self.resolution_y)
        self.widget = QWidget(self)
        self.widget.setGeometry(0,0, self.resolution_x,self.resolution_y)
        self.show()
    def Close(self):
        self.close()
    ...
    def Update(self, resolution_x, resolution_y, q_image):
        ...
        self.label.setGeometry(0, 0, self.resolution_x, self.resolution_y)
        self.widget.setGeometry(0, 0, self.resolution_x, self.resolution_y)
        self.resize(self.resolution_x, self.resolution_y)
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(QColor(127,127,127)))
        painter.drawRect(0, 0, self.resolution_x, self.resolution_y)
...
class Dialog_UI(QDialog):
    ...
    def __init__(self, parent):
        ...
        uic.loadUi(self.dir_name + '/tela_settings.ui', self)
