...
class TelaDocker(DockWidget):
    ...
    def Connects(self):
        ...
        self.dialog.record_window.toggled.connect(self.Record_Window)
    ...
    def Record_Window(self):
        self.record_window = self.dialog.record_window.isChecked()
    ...
    def Panel_Calculations(self):
        ...
        if self.timer_state == 1:
            if ((self.canvas() is not None) and (self.canvas().view() is not None)):
                self.thread.Variables(self.timer_state, self.width, self.height, self.doc_width, self.doc_height, self.interval)
            else: # No Active Document
                self.thread.Variables(0, 1, 1, 1, 1, self.interval)
    ...
    def Record_Close(self, SIGNAL_CLOSE):
        self.dialog.record_window.setChecked(False)
    # Extension
...
class Image(QThread):
    ...
    def __init__(self, parent = None):
        ...
        self.height = 0
        self.interval = 0.1
    def Variables(self, timer_state, width, height, doc_width, doc_height, interval):
